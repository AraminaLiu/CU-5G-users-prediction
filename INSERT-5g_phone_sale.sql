...
partition(day = '20210419', province = 'qh')
...
where substring(slicetime,1,8) between 20210413 and 20210419
...
sum(durition) social_durition, sum(total_traff) social_traff
from data_db.qh_lte_s1u_http
where substring(slicetime,1,8) between 20210413 and 20210419
and length(msisdn) = 11
and substring(msisdn,1,3) not in('145','146')
...
sum(durition) game_durition, sum(total_traff) game_traff
from data_db.qh_lte_s1u_http
where substring(slicetime,1,8) between 20210413 and 20210419
and length(msisdn) = 11
and substring(msisdn,1,3) not in('145','146')
and (instr(host,'sgz.ejoy.com') > 0 
...
or (instr(host,'.qq.com') > 0
and (instr(user_agent,'Survive/') > 0
...
or (instr(host,'.netease.com') > 0 
and (instr(user_agent,'xyqmobile') > 0 
or instr(host,'my.') > 0 
or instr(host,'nie') > 0 
or instr(host,'my-') > 0 
or instr(host,'gl8') > 0)) 
...
sum(durition) shop_durition, sum(data_flow) shop_traff
from unicom_oss.zqq_dw_third_level_label
where province = 'qh'
and day between 20210413 and 20210419
...
sum(durition) video_durition, sum(data_flow) video_traff
from unicom_oss.zqq_dw_third_level_label
where province = 'qh'
and day between 20210413 and 20210419
and substring(third_label_id,1,6) = '004005'
